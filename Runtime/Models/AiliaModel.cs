...
public class AiliaModel : IDisposable
{
    ...
    */
     ...
     *   Ailia.ailiaGetBlobShape() obtains the shape (layer format) of the internal data (Blob) at the time of inference and stores it in shape.
     */
    public Ailia.AILIAShape GetBlobShape(uint idx)
    {
        if (ailia == IntPtr.Zero)
        {
            return null;
        ...
        Ailia.AILIAShape shape = new Ailia.AILIAShape();
        int status = Status = Ailia.ailiaGetBlobShape(ailia, shape, idx, Ailia.AILIA_SHAPE_VERSION);
        if (status != Ailia.AILIA_STATUS_SUCCESS)
        {
            if (logging)
            {
    ...
    /**
    * \~japanese
    * \overload public Ailia.AILIAShape GetBlobShape(int idx)
    *
    * \~english
    * \overload public Ailia.AILIAShape GetBlobShape(int idx)
    */
    ...
    */
    public bool GetBlobData(float[] output_data, uint idx)
    {
        if (ailia == IntPtr.Zero)
        {
            return false;
        ...
        GCHandle output_buf_handle = GCHandle.Alloc(output_data, GCHandleType.Pinned);
        IntPtr output_buf_ptr = output_buf_handle.AddrOfPinnedObject();
        int status = Status = Ailia.ailiaGetBlobData(ailia, output_buf_ptr, (uint)(output_data.Length * 4), idx);
        output_buf_handle.Free();
    ...
    /**
    * \~japanese
    * \overload public bool GetBlobData(float[] output_data, int idx)
    *
    * \~english
    * \overload public bool GetBlobData(float[] output_data, int idx)
    */
    public bool GetBlobData(float[] output_data, int idx)
    {
        ...
        return GetBlobData(output_data, (uint)idx);
    }
     ...
     */
    public bool SetInputBlobData(float[] input_data, uint idx)
    {
        if (ailia == IntPtr.Zero)
        {
            return false;
        ...
        GCHandle input_buf_handle = GCHandle.Alloc(input_data, GCHandleType.Pinned);
        IntPtr input_buf_ptr = input_buf_handle.AddrOfPinnedObject();
        int status = Status = Ailia.ailiaSetInputBlobData(ailia, input_buf_ptr, (uint)(input_data.Length * 4), idx);
        input_buf_handle.Free();
    ...
    /**
    * \~japanese
    * \overload public bool SetInputBlobData(float[] input_data, int idx)
    *
    * \~english
    * \overload public bool SetInputBlobData(float[] input_data, int idx)
    */
    public bool SetInputBlobData(float[] input_data, int idx)
    {
        ...
        return SetInputBlobData(input_data, (uint)idx);
    }
    ...
    */
    public bool SetInputBlobShape(Ailia.AILIAShape shape, uint idx)
    {
        if (ailia == IntPtr.Zero)
        {
            return false;
        ...
        int status = Status = Ailia.ailiaSetInputBlobShape(ailia, shape, idx, Ailia.AILIA_SHAPE_VERSION);
        if (status != Ailia.AILIA_STATUS_SUCCESS)
        {
            if (logging)
            {
    ...
    /**
    * \~japanese
    * \overload public bool SetInputBlobShape(Ailia.AILIAShape shape, int idx)
    *
    * \~english
    * \overload public bool SetInputBlobShape(Ailia.AILIAShape shape, int idx)
    */
    public bool SetInputBlobShape(Ailia.AILIAShape shape, int idx)
    {
        ...
        return SetInputBlobShape(shape, (uint)idx);
    }
    ...
    */
    public bool SetInputBlobShapeND(uint[] shape, uint dim, uint idx)
    {
        if (ailia == IntPtr.Zero)
        {
            return false;
        ...
        int status = Status = Ailia.ailiaSetInputBlobShapeND(ailia, shape, dim, idx);
        if (status != Ailia.AILIA_STATUS_SUCCESS)
        {
            if (logging)
            {
    ...
    /**
    * \~japanese
    * \overload public bool SetInputBlobShapeND(uint[] shape, int dim, int idx)
    *
    * \~english
    * \overload public bool SetInputBlobShapeND(uint[] shape, int dim, int idx)
    */
    public bool SetInputBlobShapeND(uint[] shape, int dim, int idx)
    {
        ...
        return SetInputBlobShapeND(shape, (uint)dim, (uint)idx);
    }
    ...
    * @param src_model コピー元Blobが含まれるモデルオブジェクト nullを指定した場合 現在のモデルオブジェクト内でBlobをコピーします。
    * @return
    ...
    * @param src_mdoel The model object that contains source blob. If specify null, perform copy blob inner this model object.
    * @return
    *   Returns true on success, false on failure.
    */
    public bool CopyBlobData(uint dst_blob_idx, uint src_blob_idx, AiliaModel src_model = null) {
        IntPtr src_ailia = ailia;
        if (src_model != null) src_ailia = src_model.ailia;
        if (ailia == IntPtr.Zero || src_ailia == IntPtr.Zero) return false;

        int status = Status = Ailia.ailiaCopyBlobData(ailia, dst_blob_idx, src_ailia, src_blob_idx);
        if (status != Ailia.AILIA_STATUS_SUCCESS)
        {
            if (logging)
            {
